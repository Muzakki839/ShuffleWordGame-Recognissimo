...
using TMPro;
using UnityEngine.EventSystems; // DITAMBAHKAN: Untuk mengakses event handler
using System.Text.RegularExpressions;

namespace Recognissimo.Samples.SpeechRecognizerExample
{
    [AddComponentMenu("")]
    // DIUBAH: Tambahkan IPointerDownHandler dan IPointerUpHandler
    public class SpeechRecognizerExample : MonoBehaviour, IPointerDownHandler, IPointerUpHandler
    {
        ...
        private void OnEnable()
        {
            ...
            UpdateStatus("Hold button to speak"); // Pesan awal yang lebih jelas

            // Bind recognizer to event handlers.
            recognizer.Started.AddListener(() =>
            {
                _recognizedText.Clear();
                UpdateStatus("Listening..."); // Status saat mendengarkan
            });
            ...
            // DIHAPUS: Listener onClick yang lama kita hapus karena diganti dengan sistem hold.
            /*
        ...
        // DITAMBAHKAN: Method ini akan terpanggil saat tombol DITEKAN
        ...
        // DITAMBAHKAN: Method ini akan terpanggil saat tombol DILEPAS
        public void OnPointerUp(PointerEventData eventData)
        {
            // Asumsi: method untuk berhenti adalah StopProcessing()
            // Jika nama methodnya berbeda (misal: Stop(), Cancel()), ganti di bawah ini.
            recognizer.StopProcessing();
            UpdateStatus("Hold button to speak");
        }
        ...
        private void OnResult(Result result)
        {
            ...
            UpdateStatus(_recognizedText.CurrentText);
            recognizer.StopProcessing(); // Berhenti setelah mendapatkan hasil
        }
        ...
        private class RecognizedText
        {
            ...
            {
                _changingText = "";
                // Membersihkan teks dari semua tag HTML/Rich Text sebelum disimpan
                string cleanText = Regex.Replace(result.text, "<.*?>", string.Empty).Trim();
                // Menambahkan spasi hanya jika stableText tidak kosong
                if (!string.IsNullOrEmpty(_stableText))
                {
                    _stableText += " ";
                }
                _stableText += cleanText;
            }
